...
class BaseBlancoEnv(BaseTelescopeEnv, ABC):
    ...
    def _calculate_global_features(self, field_id, filter_idx, timestamp, sunset_ts, sunrise_ts, ra_arr, dec_arr):
        ...
        if field_id == ZENITH_FIELD_ID:
            ...
            if self._bin_num == WAIT_SIGNAL or (not self.do_filt):
                ...
                for filt in FILTER2WAVE.keys():
                    new_features['is_filter_{filt}'] = np.float32(filt_str == filt)
        # --- OnlineEnv Logic --- #
        ...
        observer = ephemerides.blanco_observer(time=timestamp)
        moon = ephem.Moon()
        moon.compute(observer)
        new_features['moon_phase'] = np.float32(moon.phase)
        ...
        if sunrise_ts == sunset_ts:
            ...
            # new_features['night_progress'] = 0
        else:
            new_features['night_progress'] = normalize_timestamp(timestamp, sunset_timestamp=sunset_ts, sunrise_timestamp=sunrise_ts)
...
class OfflineBlancoTestingEnv(BaseBlancoEnv):
    ...
    """
    def __init__(self, gcfg, cfg, max_nights=None, exp_time=90., global_pd_nightgroup=None, zenith_bin_states=None, fwhm_night_interps=None):
        """
        Args
        ----
        ...
        self.max_norm_feature_names = gcfg['features']['SIN_NORM_FEATURE_NAMES']
