        ...
        while not self.state.check_end_condition():
            ...
            while not submitted:
                ...
                if exposure_finished and self.state.check_start_condition():
                    ...
                    submitted = True
                    print(
                        "[Orchestrator] Observation submitted after exposure finished."
                    )
                    continue
                ...
                if (
                    time.time() - self.last_telemetry_check > 20
                ):  # XXX this amount should be an argument
                    self.last_telemetry_check = time.time()
                    new_telemetry = self.api.get_telemetry()
                    telemetry_changed = (
                        False  # XXX placeholder: check telemetry changes
                    )
